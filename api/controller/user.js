...
export const register = async (req, res, next) => {
  try {
    ...
    if (isEmail(auth)) {
      ...
      if (usedNumber) {
        return next(createError(404, "Phone number already exist"));
      } else {
        // Generate Account Activation Code
        let activationCode = getRandomCode(10000, 99999);
        const checkCode = await User.findOne({ access_token: activationCode });
        if (checkCode) {
          activationCode = getRandomCode(10000, 99999);
        }
        ...
        if (users) {
          ...
          sendOTP(
            users.phone,
            `Hi! ${users.first_name} ${users.sur_name}. Your account activation code ${activationCode}`
          );

          res
            .status(200)
            .cookie("otp", users.phone, {
              expires: new Date(Date.now() + 1000 * 60 * 15),
            })
            .json({
              message: "User successfully created",
    ...
    if (isEmail(auth)) {
      ...
      if (!emailUser) {
        ...
        if (!passwordVerify(password, emailUser.password)) {
          ...
          const token = createToken({ id: emailUser._id }, "365d");
          res
            .status(200)
      ...
      const phoneUser = await User.findOne({ phone: auth });
      if (!phoneUser) {
        next(createError(400, "User not found"));
      } else {
        if (!passwordVerify(password, phoneUser.password)) {
          next(createError(400, "Wrong password"));
        } else {
          const token = createToken({ id: phoneUser._id }, "365d");
          res
            .status(200)
            .cookie("authToken", token, {
              expires: new Date(Date.now() + 86400000 * 365),
